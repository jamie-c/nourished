...
const Form: NextPage = () => {
    ...
    if (!response?.message) {
    return (
        <form className="font-transat-bold flex flex-col gap-10" onSubmit={handleSubmit} method="POST">
            <NFCText label='First Name' placeholder='First Name:' value={formData.firstName} onChange={(e) => setFormData({...formData, firstName: e.target.value})} />
            <NFCText label='Last Name' placeholder='Last Name:' value={formData.lastName} onChange={(e) => setFormData({...formData, lastName: e.target.value})} />
            ...
            <NFCTextArea label='Message' placeholder='Your Questions or Comments:' value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} />
                {loading ? (
